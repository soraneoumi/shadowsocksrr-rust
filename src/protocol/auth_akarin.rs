...
use crate::crypto::{ChaCha20Stream, aes128_cbc_decrypt_no_padding, hmac_md5};
...
const DEFAULT_AUTH_AKARIN_MAX_TIME_DIF_SECS: i32 = 60 * 60 * 24;

#[derive(Clone, Copy)]
pub enum AuthAkarinVariant {
    Rand,
    SpecA,
}

impl AuthAkarinVariant {
    pub fn from_method(method: &str) -> Option<Self> {
        match method {
            "auth_akarin_rand" => Some(Self::Rand),
            "auth_akarin_spec_a" => Some(Self::SpecA),
            _ => None,
        }
    }

    fn default_max_time_dif(self) -> i32 {
        DEFAULT_AUTH_AKARIN_MAX_TIME_DIF_SECS
    }

    pub(crate) fn max_time_dif_from_protocol_param(self, protocol_param: &str) -> i32 {
        parse_auth_akarin_named_param::<i32>(protocol_param, &["time_dif", "max_time_dif"])
            .filter(|v| *v > 0)
            .unwrap_or_else(|| self.default_max_time_dif())
    }
}

fn parse_auth_akarin_named_param<T: std::str::FromStr>(
    protocol_param: &str,
    keys: &[&str],
) -> Option<T> {
    protocol_param.split('#').skip(1).find_map(|token| {
...
pub struct AuthAkarinCodec {
    variant: AuthAkarinVariant,
    method: String,
    key: Vec<u8>,
    recv_iv: Vec<u8>,
    ...
    crypt_tx: Option<ChaCha20Stream>,
    crypt_rx: Option<ChaCha20Stream>,

    new_send_tcp_mss: u16,
    send_tcp_mss: u16,
    recv_tcp_mss: u16,

    data_size_list: Vec<u16>,
    data_size_list2: Vec<u16>,

    shared_registry: SharedUserRegistry,
}

impl AuthAkarinCodec {
    pub fn new(
        variant: AuthAkarinVariant,
        ...
        let salt = match variant {
            AuthAkarinVariant::Rand => b"auth_akarin_rand".as_ref(),
            AuthAkarinVariant::SpecA => b"auth_akarin_spec_a".as_ref(),
        };
        ...
        let mut codec = Self {
            ...
            max_time_dif: variant.max_time_dif_from_protocol_param(&cfg.protocol_param),
            salt,
            no_compatible_method: cfg.method.clone(),
        ...
        if matches!(codec.variant, AuthAkarinVariant::SpecA) {
    ...
    fn not_match_return(&mut self, buf: &[u8]) -> (Vec<u8>, bool) {
        ...
        warn!(
            method = %self.method,
            recv_len = buf.len(),
            "auth_akarin protocol mismatch; switching to raw_trans"
        );
    ...
    fn init_data_size(&mut self, key: &[u8]) {
        ...
        let list_len2 = (random.next() % 16 + 8) as usize;
        for _ in 0..list_len2 {
    ...
    fn send_rnd_data_len(
        ...
        match self.variant {
            AuthAkarinVariant::Rand => {
                if buf_size + self.overhead as usize > self.send_tcp_mss as usize {
                    random.init_from_bin_len(last_hash, buf_size);
                    return (random.next() % 521) as usize;
                }
                if buf_size >= 1440
                    || buf_size + self.overhead as usize == self.send_tcp_mss as usize
                {
                    return 0;
                }
                random.init_from_bin_len(last_hash, buf_size);
                ...
                let span = self
                    .send_tcp_mss
                    .saturating_sub((buf_size + self.overhead as usize) as u16)
                    .max(1);
                (random.next() % span as u64) as usize
            }
            AuthAkarinVariant::SpecA => {
                if buf_size + self.overhead as usize > self.send_tcp_mss as usize {
                    random.init_from_bin_len(last_hash, buf_size);
                    return (random.next() % 521) as usize;
                }
                ...
                let pos = lower_bound(&self.data_size_list, buf_size + self.overhead as usize);
                let final_pos = pos + (random.next() as usize % self.data_size_list.len().max(1));
                if final_pos < self.data_size_list.len() {
                    return self.data_size_list[final_pos] as usize
                        - buf_size
                        - self.overhead as usize;
                }

                let pos2 = lower_bound(&self.data_size_list2, buf_size + self.overhead as usize);
                let final_pos2 =
                    pos2 + (random.next() as usize % self.data_size_list2.len().max(1));
                if final_pos2 < self.data_size_list2.len() {
                    return self.data_size_list2[final_pos2] as usize
                        - buf_size
                        - self.overhead as usize;
                }
                if final_pos2 < pos2 + self.data_size_list2.len().saturating_sub(1) {
                    return 0;
                }
    ...
    fn recv_rnd_data_len(
        ...
        match self.variant {
            AuthAkarinVariant::Rand => {
                if buf_size + self.overhead as usize > self.recv_tcp_mss as usize {
                    random.init_from_bin_len(last_hash, buf_size);
                    return (random.next() % 521) as usize;
                }
                if buf_size >= 1440
            ...
            AuthAkarinVariant::SpecA => {
                ...
                if buf_size >= 1440
                    || buf_size + self.overhead as usize == self.recv_tcp_mss as usize
                {
                    return 0;
                }
                random.init_from_bin_len(last_hash, buf_size);
    ...
    fn rnd_data(
        ...
        let rand_len = self.send_rnd_data_len(buf_size, last_hash, random);
    ...
    fn decrypt_head_block(&self, encrypted_16: &[u8], iv: &[u8; 16]) -> Result<[u8; 16]> {
        ...
        let plain = aes128_cbc_decrypt_no_padding(&password, iv, encrypted_16)?;
        let mut out = [0_u8; 16];
    ...
    fn init_connection_ciphers(&mut self) -> Result<()> {
        ...
        let tx_nonce = self.last_server_hash[..8].to_vec();
        let rx_nonce = self.last_client_hash[..8].to_vec();

        self.crypt_tx = Some(ChaCha20Stream::new_from_password(&password, &tx_nonce)?);
        self.crypt_rx = Some(ChaCha20Stream::new_from_password(&password, &rx_nonce)?);
        Ok(())
    }
...
impl ProtocolCodec for AuthAkarinCodec {
    fn decode_from_client(&mut self, input: &[u8]) -> Result<(Vec<u8>, bool)> {
        ...
        if !self.has_recv_header {
            ...
            let head = self.decrypt_head_block(&self.recv_buf[16..32], &[0_u8; 16])?;
            self.client_over_head = u16::from_le_bytes(head[12..14].try_into().unwrap());
            self.recv_tcp_mss = u16::from_le_bytes(head[14..16].try_into().unwrap());
            ...
            if time_dif < -self.max_time_dif || time_dif > self.max_time_dif {
                warn!(
                    ...
                    "auth_akarin timestamp check failed"
                );
                let recv_snapshot = self.recv_buf.clone();
                return Ok(self.not_match_return(&recv_snapshot));
            }
        ...
        loop {
            ...
            let mut data_len = u16::from_le_bytes(self.recv_buf[0..2].try_into().unwrap())
            ...
            while data_len >= 0xff00 {
                if data_len == 0xff00 {
                    ...
                    header_offset += 2;
                    if self.recv_buf.len() < header_offset + 2 {
                        return Ok((Vec::new(), false));
                    }
                    data_len =
                        u16::from_le_bytes(
                            self.recv_buf[header_offset..header_offset + 2]
                                .try_into()
                                .unwrap(),
                        ) ^ u16::from_le_bytes(self.last_client_hash[12..14].try_into().unwrap());
                } else {
                    self.raw_trans = true;
                    self.recv_buf.clear();
                    warn!(
                        ...
                        data_len,
                        "auth_akarin invalid command frame"
                    );
                    if self.recv_id == 1 {
                        return Ok((vec![b'E'; 2048], false));
                    }
                    return Err(anyhow!("auth_akarin invalid command frame"));
                }
            }

            let mut random_client = std::mem::take(&mut self.random_client);
            let last_client_hash = self.last_client_hash;
            let rand_len = self.recv_rnd_data_len(
                data_len as usize + cmd_len,
                &last_client_hash,
                &mut random_client,
            );
            ...
            if length >= 4096 {
                ...
                warn!(
                    ...
                    recv_id = self.recv_id,
                    length,
                    "auth_akarin frame length overflow"
                );
                if self.recv_id == 1 {
                    return Ok((vec![b'E'; 2048], false));
                }
                return Err(anyhow!("auth_akarin decode data length overflow"));
            }

            let frame_len = header_offset + 2 + length + 2;
            if frame_len > self.recv_buf.len() {
                break;
            }

            let data_end = header_offset + 2 + length;
            let client_hash = hmac_md5(&mac_key, &self.recv_buf[..data_end]);
            if client_hash[..2] != self.recv_buf[data_end..frame_len] {
                self.raw_trans = true;
                self.recv_buf.clear();
                warn!(
                    method = %self.method,
                    recv_id = self.recv_id,
                    "auth_akarin checksum mismatch"
                );
                if self.recv_id == 1 {
                    return Ok((vec![b'E'; 2048], false));
                }
                return Err(anyhow!("auth_akarin checksum mismatch"));
            }

            self.recv_id = self.recv_id.wrapping_add(1);
            let pos = header_offset + 2;
            let plain = self
                .crypt_rx
                .as_mut()
    ...
    fn on_encode_to_client_flushed(&mut self, delivered: bool) -> Result<()> {
        let _ = delivered;
        Ok(())
    }
...
mod tests {
    ...
    fn build_auth_akarin_client_packet(
        variant: AuthAkarinVariant,
        payload: &[u8],
        uid: u32,
    ) -> Vec<u8> {
        let cfg = base_cfg(match variant {
            AuthAkarinVariant::Rand => "auth_akarin_rand",
            AuthAkarinVariant::SpecA => "auth_akarin_spec_a",
        });
        let mut codec = AuthAkarinCodec::new(variant, cfg.clone(), SharedUserRegistry::new(64));
        assert!(codec.select_user_key(uid));

        let prefix = [2_u8, 4, 6, 8];
        let md5data = hmac_md5(&cfg.key, &prefix);

        let mut head = [0_u8; 16];
        head[0..4].copy_from_slice(&now_secs_u32().to_le_bytes());
        head[4..8].copy_from_slice(&31_u32.to_le_bytes());
        head[8..12].copy_from_slice(&47_u32.to_le_bytes());
        head[12..14].copy_from_slice(&cfg.overhead.to_le_bytes());
        head[14..16].copy_from_slice(&1460_u16.to_le_bytes());
        ...
        let mut crypt =
            ChaCha20Stream::new_from_password(&password, &md5data[..8]).expect("chacha");
        let encrypted = crypt.process(payload);

        let mut random = XorShift128Plus::default();
        let rand_len = codec.recv_rnd_data_len(encrypted.len(), &md5data, &mut random);
        let mut frame = Vec::with_capacity(2 + encrypted.len() + rand_len + 2);
    ...
    #[test]
    fn auth_akarin_roundtrip_single_user_request() {
        let cfg = base_cfg("auth_akarin_spec_a");
        let registry = SharedUserRegistry::new(64);
        let mut codec = AuthAkarinCodec::new(AuthAkarinVariant::SpecA, cfg, registry);
        let packet = build_auth_akarin_client_packet(
            AuthAkarinVariant::SpecA,
            b"\x01\x7f\x00\x00\x01\x00\x35hello",
            0,
        );
        ...
        assert!(
            sendback,
            "initial auth_akarin packet should request a response"
        );
        assert_eq!(decoded, b"\x01\x7f\x00\x00\x01\x00\x35hello");
    }

    #[test]
    fn auth_akarin_rejects_unknown_uid_when_users_are_configured() {
        let mut cfg = base_cfg("auth_akarin_spec_a");
